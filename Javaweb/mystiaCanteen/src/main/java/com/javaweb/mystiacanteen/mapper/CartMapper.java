...
public interface CartMapper extends BaseMapper<Cart> {
    @Select("SELECT name,num,type FROM cart WHERE username=#{username}")
    ...
    int findCartId(String username, String name);//找到要修改的项的ID

    @Update("UPDATE cart SET num = num + #{num} WHERE cartId=#{cartId}")
    Boolean updateNum(Integer num, Integer cartId);//修改存在的商品的数量

    @Insert("INSERT INTO cart(username,name,num) VALUES (#{usernmae},#{name},{num})")
    Boolean addCart(String username, String name, int num);//添加不存在的商品;
